        ...
        <form action="{{ route('transactions.store') }}" method="POST" id="transactionForm">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="input-group">
                        <select name="customer_id" id="customer_id" class="form-control select2" required>
                            ...
                            <option value="{{ $customer->id }}">{{ $customer->nama }} - {{ $customer->no_telp }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="lapangan_id" id="lapangan_id" class="form-control" required>
                        ...
                        <option value="{{ $lap->id }}" data-harga="{{ $lap->harga_per_jam }}">
                            {{ $lap->nama }} - {{ ucfirst(str_replace('_', ' ', $lap->jenis)) }} (Rp {{ number_format($lap->harga_per_jam, 0, ',', '.') }}/jam)
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" name="tanggal_main" id="tanggal_main" class="form-control" min="{{ date('Y-m-d') }}" required>
                </div>
            ...
            <div id="productList">
                <div class="row product-item mb-2">
                    ...
                    <div class="col-md-3">
                        <input type="number" name="products[0][qty]" class="form-control product-qty" placeholder="Qty" min="1">
                    </div>
                    <div class="col-md-3">
...
$(document).ready(function() {
    ...
    function checkAvailability() {
        ...
        if (lapangan_id && tanggal_main && jam_mulai && jam_selesai) {
            $.post('{{ route("transactions.check-availability") }}', {
                _token: '{{ csrf_token() }}',
                lapangan_id, tanggal_main, jam_mulai, jam_selesai
            }, function(response) {
    ...
    $('#addCustomerForm').submit(function(e) {
        ...
        $.post('{{ route("customers.store") }}', $(this).serialize(), function(response) {
            const newOption = new Option(response.nama + ' - ' + response.no_telp, response.id, true, true);
