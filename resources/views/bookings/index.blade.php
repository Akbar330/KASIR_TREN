                                ...
                                @foreach ($lapangans as $lap)
                                    @php
                                        $booking = $bookings->first(function ($b) use ($lap, $jam) {
                                            if (!$b || !$b->jam_mulai || !$b->jam_selesai) {
                                                return false;
                                            }

                                            $mulai = \Carbon\Carbon::parse($b->jam_mulai)->format('H:i');
                                            $selesai = \Carbon\Carbon::parse($b->jam_selesai)->format('H:i');

                                            return $b->lapangan_id == $lap->id && $jam >= $mulai && $jam < $selesai;
                                        });
                                    @endphp
                    ...
                    <tbody>
                        @forelse($bookings as $booking)
                            <tr>
                                ...
                                <td>{{ substr($booking->jam_mulai, 0, 5) }} - {{ substr($booking->jam_selesai, 0, 5) }}
                                </td>
                                <td>{{ $booking->durasi_jam }} jam</td>
                                <td>Rp {{ number_format($booking->total, 0, ',', '.') }}</td>
                                <td>
                                    <span
                                        class="badge bg-{{ $booking->status_booking == 'pending' ? 'warning' : ($booking->status_booking == 'selesai' ? 'success' : 'danger') }}">
                                        {{ ucfirst($booking->status_booking) }}
                                    </span>
                                </td>
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ route('transactions.print', $booking->id) }}"
